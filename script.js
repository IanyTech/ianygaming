...
// Maintenance popup: allow dismiss so it doesn't block interactions
function initMaintenancePopup() {
  try {
    const popup = document.getElementById('maintenancePopup');
    if (!popup) return;
    const btn = document.getElementById('closeMaintenance');
    const dismissed = (() => { try { return localStorage.getItem('iany.maintenance.dismissed') === '1'; } catch { return false; }})()
    if (dismissed) { popup.style.display = 'none'; return; }
    if (btn && !btn.__wired) {
      btn.__wired = true;
      btn.addEventListener('click', () => {
        try { localStorage.setItem('iany.maintenance.dismissed', '1'); } catch(_) {}
...
if (typeof window !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      try { removeParallaxLayers(); } catch(_) {}
      try { initMagneticHover(); } catch(_) {}
      try { initScrollReveal(); } catch(_) {}
      try { initCursorTrail(); } catch(_) {}
      try { initSparklesOnHover(); } catch(_) {}
      try { if (typeof initInfoMenu === 'function') initInfoMenu(); } catch(_) {}
      try { if (typeof initProfileMenu === 'function') initProfileMenu(); } catch(_) {}
      try { if (typeof initSettingsSection === 'function') initSettingsSection(); } catch(_) {}
      try { if (typeof initSettingsLinks === 'function') initSettingsLinks(); } catch(_) {}
      try { if (typeof initAccount === 'function') initAccount(); } catch(_) {}
      try { if (typeof initContact === 'function') initContact(); } catch(_) {}
      try { if (typeof initMaintenancePopup === 'function') initMaintenancePopup(); } catch(_) {}
      try { if (typeof initNewsletter === 'function') initNewsletter(); } catch(_) {}
